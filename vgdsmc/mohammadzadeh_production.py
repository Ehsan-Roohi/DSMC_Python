...
def _restore_moment(
    ...
    # Old development checkpoints predate the JCP temperature-noise
    # instrumentation.  Keeping their loader valid is essential for the
    # frozen prior, while every prospective JCP trajectory carries speed4.
    if "speed4" in payload:
        accumulator.speed4 = np.asarray(payload["speed4"]).copy()
    else:
        accumulator.speed4 = np.zeros_like(accumulator.energy)
    return accumulator
