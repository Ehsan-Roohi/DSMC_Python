...
def test_accumulator_records_exact_fourth_raw_speed_moment():
    ...
    represented = cfg.real_particles_per_sim_particle
    expected = represented * np.sum(np.sum(velocity**2, axis=1) ** 2)
    assert np.isclose(accumulator.speed4[0], expected, rtol=1.0e-14)
