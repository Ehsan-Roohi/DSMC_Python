...
def run_pipeline(
    ...
    *,
    ...
    dsmc_cfg = PhysicalCavityConfig(
        nx=nx, ny=ny, particles_per_cell=particles_per_cell,
        knudsen=knudsen, t_left=t_left, t_right=t_right,
        t_top=t_top, t_bottom=t_bottom, steps=dsmc_steps,
        sample_start=max(1, dsmc_steps // 2), seed=seed,
    )
...
def main() -> None:
    ...
    print(json.dumps(run_pipeline(
        args.output_dir, nx=args.nx, ny=args.ny,
