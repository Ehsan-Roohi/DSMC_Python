...
def validate_stage115_design(**overrides: object) -> None:
    ...
    if any(key not in frozen or frozen[key] != value for key, value in overrides.items()):
        raise ValueError(
            "Stage 115 is frozen to the exact completed Stage-114 artifact and its mixed-structure route. "
    ...
    if RULE != (40, 96) or ANGULAR_SECTORS != 8 or INTERIOR_EXTENT != 56:
        raise ValueError("Stage 115 requires the exact 40x96 velocity rule, eight sectors, and 56x56 interior")
...
def _load_stage114(root: str | Path, record_path: str | Path) -> tuple[dict[str, object], dict[str, np.ndarray], dict[str, object]]:
    root = Path(root)
    expected = {
        "summary.json": STAGE114_SUMMARY_SHA256,
        "wall_distance_velocity_sector_maps.npz": STAGE114_MAPS_SHA256,
    }
    for name, digest in expected.items():
        path = root / name
        if not path.is_file() or sha256_file(path) != digest:
            raise ValueError(f"Stage-114 checksum mismatch: {name}")

    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    record = json.loads(Path(record_path).read_text(encoding="utf-8"))
    if summary.get("stage") != 114 or summary.get("decision") != STAGE114_DECISION or summary.get("finite") is not True:
        raise ValueError("Stage-114 artifact does not authorize the Stage-115 distribution-specific audit")
    if float(summary.get("max_parent_x_change_closure_relative_l2", np.inf)) > PROFILE_RECONSTRUCTION_TOLERANCE:
        raise ValueError("Stage-114 parent-map closure is not admissible")
    if record.get("stage") != 114 or record.get("decision") != STAGE114_DECISION:
    ...
    with np.load(root / "wall_distance_velocity_sector_maps.npz") as data:
        if not needed.issubset(data.files):
            raise ValueError("Stage-114 payload misses required sector or growth maps")
        arrays = {name: np.asarray(data[name], dtype=np.float64).copy() for name in needed}

    for name in ("phi_x_sector_change", "psi_x_sector_change"):
        value = arrays[name]
        if value.shape != (ANGULAR_SECTORS, INTERIOR_EXTENT, INTERIOR_EXTENT) or not np.isfinite(value).all() or np.any(value < 0.0):
            raise ValueError(f"Stage-114 map {name} is invalid")
    for name in ("phi_growth_amplitude", "psi_growth_amplitude", "joint_growth_amplitude"):
        value = arrays[name]
        if value.shape != (INTERIOR_EXTENT, INTERIOR_EXTENT) or not np.isfinite(value).all() or np.any(value < 0.0):
            raise ValueError(f"Stage-114 growth map {name} is invalid")
    if arrays["sector_center_degrees"].shape != (ANGULAR_SECTORS,):
        raise ValueError("Stage-114 sector-center vector is invalid")
...
def _profile_metrics(phi: np.ndarray, psi: np.ndarray) -> dict[str, object]:
    ...
    ps = float(np.sum(p)); qs = float(np.sum(q))
    if ps <= 0.0 or qs <= 0.0:
        raise ValueError("Stage-115 profiles must have positive mass")
    p = p / ps; q = q / qs
    ...
    m = 0.5 * (p + q)
    mask_p = p > 0.0; mask_q = q > 0.0
    kl_p = float(np.sum(p[mask_p] * np.log2(p[mask_p] / m[mask_p])))
    kl_q = float(np.sum(q[mask_q] * np.log2(q[mask_q] / m[mask_q])))
    js_bits = 0.5 * (kl_p + kl_q)
    phi_top2 = _top2(p); psi_top2 = _top2(q); joint = 0.5 * (p + q); joint_top2 = _top2(joint)
    ...
    return {
        ...
        "joint_top2_share": float(np.sum(joint[joint_top2])),
...
def stage115_decision(metrics: dict[str, dict[str, object]], finite: bool, max_profile_reconstruction_error: float) -> str:
    ...
    broad_metrics = [metrics["near_1_4"], metrics["mid_5_14"]]
    common_pair_sets = []
    common = True
    for block in broad_metrics:
        pair = tuple(sorted(int(v) for v in block["joint_top2_sector_index"]))
        common_pair_sets.append(pair)
        ...
        if float(block["phi_top2_share"]) < COMMON_PAIR_SHARE_MIN or float(block["psi_top2_share"]) < COMMON_PAIR_SHARE_MIN:
            common = False
    if common and common_pair_sets[0] == common_pair_sets[1]:
        return "stage115_common_adjacent_pair_support_stage116_pair_resolved_radial_node_audit"

    divergent = any(
        float(block["profile_cosine"]) <= DIVERGENCE_COSINE_MAX
        or float(block["overlap_coefficient"]) <= DIVERGENCE_OVERLAP_MAX
        for block in broad_metrics
    )
    if divergent:
...
def run_stage115(stage114_artifact_dir: str | Path, stage114_record_path: str | Path, output_dir: str | Path, **design: object) -> dict[str, object]:
    validate_stage115_design(**design)
    stage114_summary, arrays, stage114_record = _load_stage114(stage114_artifact_dir, stage114_record_path)

    phi_profiles = _band_profiles(arrays["phi_x_sector_change"], arrays["phi_growth_amplitude"])
    psi_profiles = _band_profiles(arrays["psi_x_sector_change"], arrays["psi_growth_amplitude"])
    joint_profiles = 0.5 * (phi_profiles + psi_profiles)

    errors: list[float] = []
    metrics: dict[str, dict[str, object]] = {}
    for iband, band in enumerate(BANDS):
        expected_phi = np.asarray(stage114_summary["metrics"]["phi"][band]["sector_share"], dtype=np.float64)
        expected_psi = np.asarray(stage114_summary["metrics"]["psi"][band]["sector_share"], dtype=np.float64)
        errors.extend([
            float(np.max(np.abs(phi_profiles[iband] - expected_phi))),
            float(np.max(np.abs(psi_profiles[iband] - expected_psi))),
        ])
        block = _profile_metrics(phi_profiles[iband], psi_profiles[iband])
        block["phi_sector_share"] = phi_profiles[iband].tolist()
        block["psi_sector_share"] = psi_profiles[iband].tolist()
        block["joint_sector_share"] = joint_profiles[iband].tolist()
        metrics[band] = block

    max_error = float(max(errors))
    finite = bool(
        np.isfinite(phi_profiles).all()
        and np.isfinite(psi_profiles).all()
        and np.isfinite(joint_profiles).all()
        and np.isfinite(max_error)
    )
    decision = stage115_decision(metrics, finite, max_error)
    ...
    np.savez_compressed(
        ...
        sector_center_degrees=arrays["sector_center_degrees"],
    )

    broad_blocks = [metrics["near_1_4"], metrics["mid_5_14"]]
    broad_pair = [int(v) for v in broad_blocks[0]["joint_top2_sector_index"]]
    summary = {
        "stage": 115,
        "description": (
            "Frozen distribution-specific comparison of the Stage-114 phi and psi eight-sector profiles. "
            "The audit tests whether the mixed Stage-114 single-sector decision reflects genuine phi/psi angular divergence "
            "or a common adjacent two-sector support that sits near the preregistered single-sector threshold."
        ),
        "configuration": {
            "grid": list(GRID),
            "kn0": KNUDSEN,
            ...
            "profile_reconstruction_tolerance": PROFILE_RECONSTRUCTION_TOLERANCE,
            "stage114_run_id": STAGE114_RUN_ID,
            "stage114_job_id": STAGE114_JOB_ID,
        ...
        "stage114_authorization": {
            "decision": stage114_summary["decision"],
            "workflow_run_id": STAGE114_RUN_ID,
            "workflow_job_id": STAGE114_JOB_ID,
            "artifact_id": STAGE114_ARTIFACT_ID,
            "tests_passed": stage114_record["tests"]["passed"],
            "tests_failed": stage114_record["tests"]["failed"],
            "max_parent_x_change_closure_relative_l2": stage114_summary["max_parent_x_change_closure_relative_l2"],
        },
        "finite": finite,
        "max_stage114_profile_reconstruction_abs_error": max_error,
        "metrics": metrics,
        "broad_reference_joint_top2_sector_index": broad_pair,
        "decision": decision,
        "scientific_conclusion": (
            "Stage 115 distinguishes distribution-specific angular divergence from a common adjacent-pair support without changing the velocity quadrature. "
            "A common-pair outcome means only that the normalized phi and psi diagnostic profiles share support; it is not a causal or accuracy result."
        ),
        "negative_result_guard": (
            "Stage 115 is an artifact-only comparison of frozen Stage-114 phi/psi angular profiles. It cannot establish limiter causality, nonlinear MUSCL stability, endpoint convergence, heat-flux improvement, benchmark improvement, or external validation. "
            "Stage 114 did not support a single common sector above its preregistered threshold; Stage 113 remains a broad wall-distance localization surrogate; Stage 111 remains association rather than causal isolation; Stage 110 remains confounded by same-sign gradient strength; Stage 99 remains a negative cross-run reproducibility result; Stage 98 remains a negative cross-run replay result; Stage 90 remains nonconverged in both reconstruction arms; Stage 28 remains a failed MUSCL endpoint; the Stage-89 one-sided boundary slope remains unpromoted. "
            "No failed parameter or velocity quadrature is retuned, no cross-Knudsen MUSCL extension is permitted, and no stability, accuracy, benchmark, heat-flux-improvement, or validation claim is authorized."
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("--stage114-artifact-dir", required=True)
    parser.add_argument("--stage114-record-path", required=True)
    parser.add_argument("--output-dir", required=True)
    args = parser.parse_args()
    run_stage115(args.stage114_artifact_dir, args.stage114_record_path, args.output_dir)
