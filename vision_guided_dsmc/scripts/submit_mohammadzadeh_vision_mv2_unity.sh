...
if [[ ! -x "${REPO_ROOT}/.venv-mv2/bin/python" ]]; then
  "${PYTHON_BIN}" -m venv "${REPO_ROOT}/.venv-mv2"
fi
source "${REPO_ROOT}/.venv-mv2/bin/activate"
python -m pip install --upgrade pip setuptools wheel
python -m pip install -e '.[ml,plot]'

EXPORTS="ALL,MV2_REPO_ROOT=${REPO_ROOT},MV2_M3_ROOT=${M3_ROOT},MV2_OUTPUT_ROOT=${OUTPUT_ROOT}"
...
printf 'ARRAY_JOB_ID=%q\nPOST_JOB_ID=%q\nMV2_REPO_ROOT=%q\nMV2_M3_ROOT=%q\nMV2_OUTPUT_ROOT=%q\n' \
  "${ARRAY_JOB_ID}" "${POST_JOB_ID}" "${REPO_ROOT}" "${M3_ROOT}" "${OUTPUT_ROOT}" > "${ENV_FILE}"
