...
try:
    ...
    from .r13_grid_transfer import interpolate_interior_nodal_state
...
def run(
    ...
    *,
    ...
    if initial is not None:
        ...
        target_shape = (config.ny, config.nx, NVAR)
        if initial.shape != target_shape:
            if initial.ndim != 3 or initial.shape[2] != NVAR:
                raise ValueError(
                    f"initial state shape {initial.shape} cannot be transferred "
                    f"to {target_shape}"
                )
            initial, grid_transfer = interpolate_interior_nodal_state(
                initial,
                target_nx=config.nx,
                target_ny=config.ny,
                rho_index=IDX["rho"],
                theta_index=IDX["theta"],
            )
            initial_provenance["grid_transfer"] = grid_transfer
        else:
            initial_provenance["grid_transfer"] = {
                "applied": False,
                "semantics": "source_state_used_without_grid_transfer",
                "source_shape": list(initial.shape),
