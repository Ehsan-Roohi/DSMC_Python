...
def test_secant_predictor_uses_log_positive_fields_and_preserves_mass() -> None:
    previous = _case(5).equilibrium_state()
    ...
    previous[..., 0] = np.exp(0.05 * x)
    current[..., 0] = np.exp(0.10 * x)
    previous[..., 3] = np.exp(0.02 * y)
    current[..., 3] = np.exp(0.04 * y)
    previous[..., 16] = 0.01 * x
    current[..., 16] = 0.02 * x

    predicted = secant_predict_state(previous, current, 1.5)

    from r26_discretization import trapezoidal_node_weights

    weights = trapezoidal_node_weights(5)
    assert np.all(predicted[..., 0] > 0.0)
    assert np.all(predicted[..., 3] > 0.0)
    assert np.isclose(np.sum(weights * predicted[..., 0]), 1.0, atol=2.0e-14)
    assert np.allclose(predicted[..., 3], np.exp(0.07 * y))
    assert np.allclose(predicted[..., 16], 0.035 * x)
...
def test_colored_newton_falls_back_when_sparse_lu_direction_is_not_descent() -> None:
    """A bad LU direction must activate the fail-closed LSMR ladder."""
    ...
    class BadFactorization:
        def solve(self, right_hand_side: np.ndarray) -> np.ndarray:
            # The production call supplies -residual.  Return +residual so the
            # primary direction is provably uphill and cannot pass Armijo.
            return -np.asarray(right_hand_side, dtype=float)

    with patch.object(solver_module, "splu", return_value=BadFactorization()):
        result = solve_r26_bvp(
            problem,
            state,
            options=SolveOptions(
                ...
                held_out_continuity_tolerance=1.0e-10,
                max_iterations=5,
                lsmr_max_iterations=80,
            ),
        )

    assert result.converged, result.message
    assert result.scipy_success
    assert "globalization fallbacks accepted=" in result.message
