...
small optimizer norm to model validation.  The manuscript target uses the Gu
equilibrium mean-free-path convention, argon VHS omega=0.81, Tw=300 K and a
100 m/s lid.  The legacy Rana/John and Gu-ASME modes are retained only for
regression comparisons and must be selected explicitly.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--case-family",
        choices=("jfm-observability", "rana-john2010", "gu-asme2009"),
        default="jfm-observability",
        help=(
            "JFM-2026-1451 DSMC target, Rana/John comparison, or direct "
            "source-locked Gu-ASME case"
        ),
    )
    ...
    parser.add_argument("--vhs-omega", type=float, default=0.81)
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "older accepted restart used with --initial-state for a guarded "
            "secant predictor in Kn"
        ),
    )
    parser.add_argument(
        "--secant-max-factor",
        type=float,
        default=2.0,
        help="maximum extrapolation factor before merit-guarded backtracking",
    )
    parser.add_argument(
        "--reconcile-initial",
        action="store_true",
    ...
    if args.previous_state is not None and args.initial_state is None:
        parser.error("--previous-state requires --initial-state")
    if not np.isfinite(args.secant_max_factor) or args.secant_max_factor <= 0.0:
        parser.error("--secant-max-factor must be finite and positive")
    if args.case_family == "jfm-observability":
        ...
        base = jfm_observability_cavity_case(
            ...
            viscosity_exponent=args.vhs_omega,
    ...
    if args.initial_state is None:
        ...
        with np.load(args.initial_state, allow_pickle=False) as archive:
            ...
            current_kn = float(archive["kn_input"]) if "kn_input" in archive else None
            current_kn_convention = (
        ...
        if current_kn_convention is not None and current_kn_convention != base.kn_convention.value:
            parser.error("initial-state Kn convention does not match the target case")
        kn_changed = bool(
            current_kn is not None
            and not np.isclose(current_kn, base.kn, rtol=0.0, atol=1.0e-15)
        )
        provenance["restart_kn_input"] = current_kn
        provenance["kn_changed"] = kn_changed

        if args.previous_state is not None:
            if current_kn is None:
                parser.error("secant prediction requires kn_input in --initial-state")
            with np.load(args.previous_state, allow_pickle=False) as archive:
                if "accepted" in archive and not bool(np.asarray(archive["accepted"]).item()):
                    parser.error("previous-state archive is explicitly marked rejected")
                previous_old = np.asarray(archive["state"], dtype=float)
                previous_x = np.asarray(archive["x"], dtype=float)
                previous_y = np.asarray(archive["y"], dtype=float)
                previous_lid = float(archive["lid_velocity"])
                if "kn_input" not in archive:
                    parser.error("secant prediction requires kn_input in --previous-state")
                previous_kn = float(archive["kn_input"])
                previous_kn_convention = (
                    str(np.asarray(archive["kn_convention"]).item())
                    if "kn_convention" in archive
                    else None
                )
            if previous_kn_convention is not None and previous_kn_convention != base.kn_convention.value:
                parser.error("previous-state Kn convention does not match the target case")
            if not np.isclose(previous_lid, current_lid, rtol=0.0, atol=1.0e-14):
                parser.error("secant restart states must have the same accepted lid velocity")
            if not previous_kn < current_kn < base.kn:
                parser.error("secant Kn values must satisfy previous < current < target")

            weights = wall_bounded_control_volume_weights(base.x, base.y)
            previous = interpolate_state_grid(
                previous_old,
                base.nodes,
                old_x=previous_x,
                old_y=previous_y,
            ...
            predictor_case = base.with_lid_velocity(current_lid, suffix="secant-guard")
            predictor_problem = make_problem(predictor_case)

            def predictor_merit(candidate: np.ndarray) -> float:
                try:
                    residual = predictor_problem.residual(candidate)
                except (FloatingPointError, ValueError, OverflowError):
                    return float("inf")
                return 0.5 * float(np.dot(residual, residual))

            requested_factor = (base.kn - current_kn) / (current_kn - previous_kn)
            trial_factor = min(float(requested_factor), float(args.secant_max_factor))
            best_state = state
            best_factor = 0.0
            best_merit = predictor_merit(state)
            trial_records: list[dict[str, float | bool]] = []
            for _ in range(10):
                candidate = secant_predict_state(
                    previous,
                    state,
                    trial_factor,
                    target_mean_density=base.mean_density,
                    mass_weights=weights,
                )
                merit = predictor_merit(candidate)
                improved = bool(np.isfinite(merit) and merit < best_merit)
                trial_records.append(
                    {"factor": trial_factor, "merit": merit, "improved": improved}
                )
                if improved:
                ...
                trial_factor *= 0.5
            state = best_state
            provenance["secant_predictor"] = {
                "previous_path": str(args.previous_state.resolve()),
                "previous_file_sha256": sha256_file(args.previous_state),
                "previous_kn": previous_kn,
                "current_kn": current_kn,
                "target_kn": base.kn,
                "requested_factor": requested_factor,
                "selected_factor": best_factor,
                "selected_merit": best_merit,
                "trials": trial_records,
            }
            provenance["state_sha256_after_predictor_guard"] = state_sha256(state)

        needs_reconciliation = bool(grid_changed or kn_changed or args.reconcile_initial)
    ...
    while needs_reconciliation or current_lid < target_lid - 1.0e-15:
        ...
        if accepted:
            ...
            if reconciling_this_attempt:
                termination = "restart_reconciliation_rejected"
                break
