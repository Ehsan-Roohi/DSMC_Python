...
STAGE = "MV17B_A2_Mohammadzadeh_locked_window_recovery"
AMENDMENT = "mv17b_a2_locked_window_recovery_amendment.json"
LOCK_FILE = "MV17B_A2_LOCKED_WINDOW_RECOVERY_LOCK.json"
LOCK_MANIFEST = "MV17B_A2_LOCKED_WINDOW_RECOVERY_LOCK.sha256"
AMENDMENT_COPY = "MV17B_A2_LOCKED_WINDOW_RECOVERY_AMENDMENT.json"
RECOVERY_PROVENANCE = "MV17B_A2_RECOVERY_PROVENANCE.json"
CODE_MANIFEST = "MV17B_A2_CODE_SHA256SUMS.txt"
SOURCE_PATCH_REPORT = "MV17B_A2_SOURCE_PATCH_REPORT.json"
...
PATCHED_LOOP = "DO WHILE ((TIME < TLIM) .OR. (NOUT < 116))"
PATCH_MARKER = "MV17B_A2_LOCKED_WINDOW_EXTENSION"
...
def patch_locked_window_source(source: Path, output: Path, report: Path) -> dict[str, Any]:
    """Extend only the fresh-run termination guard to the locked NOUT endpoint."""
    ...
    if PATCH_MARKER in text:
        raise ValueError("source was already patched for MV17B-A2")
    ...
    replacement = (
        f"! {PATCH_MARKER}_BEGIN\n"
        f"{PATCHED_LOOP}\n"
        f"! {PATCH_MARKER}_END"
    )
    patched = text.replace(ORIGINAL_LOOP, replacement, 1)
    if patched.count(PATCHED_LOOP) != 1 or patched.count(PATCH_MARKER) != 2:
        raise ValueError("locked-window source patch was not unique")
...
def lock_recovery(output_root: Path, amendment_path: Path) -> dict[str, Any]:
    ...
    if amendment.get("status") != "locked_after_A1_mechanical_failure_before_any_MV17B_prediction_or_target_construction":
        raise ValueError("MV17B-A2 amendment is not locked")
    ...
    for row in inventory:
        ...
        if row["status"] != expected_status:
            raise ValueError(f"unexpected pre-A2 status: {row['case_id']}")
    ...
    if copied_amendment.exists():
        if copied_amendment.read_bytes() != amendment_path.read_bytes():
            raise FileExistsError("existing A2 amendment differs")
    else:
        copied_amendment.write_bytes(amendment_path.read_bytes())
    value = {
        "stage": STAGE,
        "status": "MV17B_A2_locked_before_recovery_and_before_prediction",
    ...
    if lock.exists():
        if _json(lock) != value:
            raise FileExistsError("existing MV17B-A2 lock differs")
    else:
...
def verify_ready(output_root: Path) -> dict[str, Any]:
    root = Path(output_root).resolve()
    if (root / "analysis").exists():
        raise ValueError("analysis directory existed before A2 verification")
    lock = root / LOCK_FILE
    manifest = root / LOCK_MANIFEST
    if not lock.is_file() or not manifest.is_file():
        raise FileNotFoundError("MV17B-A2 lock is absent")
    ...
    for row in rows:
        ...
        if row["array_index"] in RECOVERY_INDICES:
            if not (case / "results" / "MV17B_A2_RECOVERY_PASS").is_file():
                raise FileNotFoundError(f"A2 recovery attestation absent: {row['case_id']}")
    recovery_files: list[Path] = [
        root / AMENDMENT_COPY,
        root / LOCK_FILE,
        root / LOCK_MANIFEST,
        root / "MV17B_A2_SUBMISSION.env",
        root / CODE_MANIFEST,
        root / "campaign" / "build" / SOURCE_PATCH_REPORT,
        root / "campaign" / "build" / "DS2V_BIRD_MV17B_A2.F90",
        root / "campaign" / "build" / "ds2v_bird_mv17b_a2",
        root / "campaign" / "build" / "MV17B_A2_BUILD_CONFIGURATION.txt",
    ]
    for row in rows:
        if row["array_index"] not in RECOVERY_INDICES:
            continue
        case = root / "campaign" / "cases" / row["case_id"]
        recovery = root / "campaign" / "recovery_a2" / row["case_id"]
        recovery_files.extend(
            (
                case / "results" / "RUN_STATUS.pre_mv17b_a2.env",
                case / "results" / "MV17B_A2_RECOVERY_METADATA.json",
                case / "results" / "MV17B_A2_RECOVERY_PASS",
                recovery / "MV17B_A2_RECOVERY_METADATA.json",
    ...
    if missing:
        raise FileNotFoundError(f"A2 recovery provenance is incomplete: {missing}")
    provenance = {
        "stage": STAGE,
        "status": "MV17B_A2_recovery_provenance_verified_before_analysis",
        "files": {
    ...
    return {
        "stage": STAGE,
        "status": "MV17B_A2_all_original_locked_blocks_ready",
...
def augment_package(output_root: Path, archive: Path) -> dict[str, Any]:
    ...
    if provenance.get("status") != "MV17B_A2_recovery_provenance_verified_before_analysis":
        raise ValueError("A2 recovery provenance was not verified")
        ...
        for path in paths:
            ...
            if relative in existing:
                ...
                if archived_digest != _sha256(path):
                    raise ValueError(f"A2 provenance archive collision: {relative}")
                continue
    ...
    returned.update(
        {
            ...
            "MV17B_A2_locked_window_recovery": True,
    ...
    return {
        "stage": STAGE,
        "status": "MV17B_A2_provenance_appended_to_result_archive",
        "archive": str(archive),
        "archive_sha256": digest,
