...
SCRIPT = (
    ...
    / "vgdsmc"
    / "mohammadzadeh_mv8_kinetic_moments.py"
)
SPEC = importlib.util.spec_from_file_location("mv8_kinetic", SCRIPT)
assert SPEC is not None and SPEC.loader is not None
mv8 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(mv8)
...
def test_additive_moments_recover_stress_and_heat_flux():
    ...
    outputs, auxiliary, diagnostics = mv8.moment_fields(payload, Config(), kb)
...
def test_merging_additive_blocks_recovers_full_payload():
    ...
    for name in ("simulated_count", "m0", "m1", "m2", "energy", "energy_velocity"):
        first[name] *= 0.5
        second[name] *= 0.5
    merged = mv8.merge_moment_payloads((first, second))
...
def test_componentwise_metric_is_scale_safe_at_zero_crossings():
    ...
    y[:, 2:, :, 2] = 0.0
    prediction = y + 0.1
    metric = mv8.field_metrics(prediction, y)
    assert np.isfinite(metric["composite_nrmse"])
    assert set(metric["per_field_nrmse"]) == set(mv8.OUTPUT_FIELDS)
...
def test_physical_figure_has_vector_and_600_dpi_outputs(tmp_path):
    ...
    record = mv8._physical_figure(tmp_path, 0, methods, reference, 20.0)
...
def test_protocol_is_explicitly_exploratory_and_forbids_local_percent_error():
    ...
    assert protocol["stage"] == mv8.STAGE
