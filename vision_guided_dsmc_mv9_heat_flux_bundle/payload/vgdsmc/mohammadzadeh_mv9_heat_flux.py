"""MV9 precision-safe, cross-seed denoising of DSMC heat-flux fields.

The stage reads immutable additive accumulators from completed DSMC
checkpoints and never launches a DSMC trajectory.  Heat-flux provenance is
audited entirely in float64 before any training tensor is cast to float32.
Independent-seed, leave-one-out targets prevent self-target leakage.
"""
...
STAGE = "MV9_Mohammadzadeh_heat_flux_noise2noise_pilot"
STATUS = "locked_before_any_MV9_heat_flux_model_outcome"
PROTOCOL_FILE = "mv9_heat_flux_noise2noise_protocol.json"
OUTPUT_FIELDS = (
    "tau_xy_over_p_ref",
...
AUDIT_DTYPE = np.float64
CHANNEL_LOSS_WEIGHTS = np.asarray((0.75, 0.75, 1.50, 2.00), dtype=np.float32)
...
def _project_modules():
    ...
    from . import mohammadzadeh_mv8_kinetic_moments as mv8
...
def locked_protocol() -> dict[str, Any]:
    ...
    if value.get("stage") != STAGE or value.get("status") != STATUS:
        raise ValueError("MV9 kinetic-moment protocol is absent or unlocked")
    ...
    if (
        ...
        or tuple(float(item) for item in training["channel_weights_tau_normal_qx_qy"])
        != tuple(float(item) for item in CHANNEL_LOSS_WEIGHTS)
        or float(training["gradient_loss_weight"]) != GRADIENT_LOSS_WEIGHT
        or bool(training["adversarial_perceptual_or_diffusion_loss"])
    ):
        raise ValueError("MV9 code differs from the locked protocol")
    ...
    checks = {
        ...
        "mv8_protocol_sha256": root / "mv8_kinetic_moment_feasibility_protocol.json",
        "mv8_module_sha256": Path(modules["mv8"].__file__),
    }
    for key, source_path in checks.items():
        if _sha256(source_path) != source[key]:
            raise ValueError(f"MV9 source ancestry hash mismatch: {key}")
    return value
...
def verify_lock() -> dict[str, Any]:
    ...
    return {
        "stage": STAGE,
        "status": "MV9_kinetic_moment_lock_verified_without_reading_model_outcomes",
        "protocol_sha256": _sha256(protocol_path()),
        "model_tasks": protocol["execution_matrix"]["model_tasks"],
        "outputs": list(OUTPUT_FIELDS),
        "primary_endpoint": protocol["analysis_contract"]["primary_endpoint"],
        "provenance_dtype": str(np.dtype(AUDIT_DTYPE)),
...
def task_from_index(index: int) -> tuple[str, int]:
    ...
    if not 0 <= index < total:
        raise ValueError(f"MV9 model task index must be in [0,{total - 1}]")
    return (
...
def _verify_source_artifacts(directory: Path) -> dict[str, Any]:
    ...
    for name in required:
        ...
        if (
            ...
            raise ValueError(f"MV9 source artifact verification failed: {path}")
    summary = json.loads((directory / "summary.json").read_text(encoding="utf-8"))
    if "config" not in summary or not str(summary.get("status", "")).startswith("complete_"):
        raise ValueError(f"MV9 source summary is not complete: {directory}")
    return summary
...
def moment_fields(
    ...
    kb: float,
    *,
    output_dtype: Any | None = TRAINING_DTYPE,
) -> tuple[np.ndarray, np.ndarray, dict[str, float]]:
    """Finalize moments in float64 and cast only when explicitly requested.

    The MV8 audit multiplied float32-normalized heat flux by ``q_ref`` before
    comparing it with a stored float64 field.  That round trip imposed a
    roughly half-single-precision-epsilon discrepancy.  MV9 keeps the entire
    provenance path in float64; the optional cast is only for neural tensors.
    """
    samples = int(payload["samples"])
    if samples <= 0:
        raise ValueError("moment payload has no samples")
    ncell = int(cfg.nx) * int(cfg.ny)
    m0 = np.asarray(payload["m0"], dtype=AUDIT_DTYPE)
    m1 = np.asarray(payload["m1"], dtype=AUDIT_DTYPE)
    m2 = np.asarray(payload["m2"], dtype=AUDIT_DTYPE)
    energy = np.asarray(payload["energy"], dtype=AUDIT_DTYPE)
    energy_velocity = np.asarray(payload["energy_velocity"], dtype=AUDIT_DTYPE)
    if (
        m0.shape != (ncell,)
        or m1.shape != (ncell, 3)
    ...
    if not np.all(np.isfinite(outputs)) or not np.all(np.isfinite(auxiliary)):
        raise ValueError("non-finite finalized MV9 moment field")
    if output_dtype is not None:
        outputs = outputs.astype(output_dtype)
        auxiliary = auxiliary.astype(output_dtype)
...
def _relative_array_difference(first: np.ndarray, second: np.ndarray) -> float:
    first64 = np.asarray(first, dtype=AUDIT_DTYPE)
    second64 = np.asarray(second, dtype=AUDIT_DTYPE)
    if first64.shape != second64.shape:
        raise ValueError("heat-flux provenance arrays have different shapes")
    if not np.all(np.isfinite(first64)) or not np.all(np.isfinite(second64)):
        raise ValueError("heat-flux provenance arrays contain non-finite values")
    scale = max(float(np.max(np.abs(second64))), 1.0)
    return float(np.max(np.abs(first64 - second64)) / scale)
...
def load_moment_source(directory: Path) -> dict[str, Any]:
    ...
    if not isinstance(block_mapping, Mapping) or len(block_mapping) != 10:
        raise ValueError(f"MV9 requires ten additive checkpoint blocks: {directory}")
    ...
    full64, full_aux64, full_diagnostics = moment_fields(
        full_payload, cfg, modules["KB"], output_dtype=None
    )
    block_outputs64, block_auxiliary64, diagnostics = [], [], []
    for payload in block_payloads:
        output, auxiliary, record = moment_fields(
            payload, cfg, modules["KB"], output_dtype=None
        )
        block_outputs64.append(output)
        block_auxiliary64.append(auxiliary)
        diagnostics.append(record)

    with np.load(directory / "fields.npz", allow_pickle=False) as stored:
        q_full = np.stack(
            (
                np.asarray(stored["qx"], dtype=AUDIT_DTYPE),
                np.asarray(stored["qy"], dtype=AUDIT_DTYPE),
            )
        )
        q_full_stored_dtypes = (str(stored["qx"].dtype), str(stored["qy"].dtype))
    with np.load(directory / "block_fields.npz", allow_pickle=False) as stored:
        ...
        q_block_stored_dtypes = (str(stored["qx"].dtype), str(stored["qy"].dtype))
    scale = np.asarray(
        [full_diagnostics["q_ref_W_m2"], full_diagnostics["q_ref_W_m2"]]
    )[:, None, None]
    reconstructed_q_full = full64[2:] * scale
    reconstructed_q_blocks = (
        np.stack(block_outputs64)[:, 2:]
        * scale[None]
    )
    ...
    full = full64.astype(TRAINING_DTYPE)
    full_aux = full_aux64.astype(TRAINING_DTYPE)
    block_outputs = np.stack(block_outputs64).astype(TRAINING_DTYPE)
    block_auxiliary = np.stack(block_auxiliary64).astype(TRAINING_DTYPE)
    return {
        "directory": directory,
        ...
        "block_auxiliary": block_auxiliary,
        "scales": np.asarray(
            [
        ...
        "q_provenance_computation_dtype": str(np.dtype(AUDIT_DTYPE)),
        "q_full_stored_dtypes": q_full_stored_dtypes,
        "q_block_stored_dtypes": q_block_stored_dtypes,
        "training_tensor_dtype": str(np.dtype(TRAINING_DTYPE)),
...
def _leave_one_out(values: Mapping[int, np.ndarray]) -> dict[int, np.ndarray]:
    seeds = tuple(values)
    if len(seeds) < 3:
        raise ValueError("MV9 leave-one-out target requires at least three seeds")
    return {
        seed: np.mean(
...
def field_metrics(candidate: np.ndarray, target: np.ndarray) -> dict[str, Any]:
    ...
    if candidate.shape != target.shape or candidate.ndim != 4:
        raise ValueError("MV9 metric arrays must be matching NCHW tensors")
    ...
    return {
        ...
        "heat_flux_composite_nrmse": float(
            np.mean(
                (
                    per_field["qx_over_q_ref"],
                    per_field["qy_over_q_ref"],
                )
            )
        ),
    }
...
def run_assembly(
    ...
    print("STAGE=MV9_additive_moment_source_audit", flush=True)
            ...
            audit_rows.append(
                {
                    ...
                    "q_provenance_computation_dtype": source[
                        "q_provenance_computation_dtype"
                    ],
                    "q_full_stored_dtypes": json.dumps(
                        source["q_full_stored_dtypes"]
                    ),
                    "q_block_stored_dtypes": json.dumps(
                        source["q_block_stored_dtypes"]
                    ),
    ...
    decision = (
        "proceed_to_MV9_B1_kinetic_models"
        if all(checks.values())
        else "hold_MV9_models_return_information_audit_only"
    )
    ...
    summary = {
        "stage": STAGE,
        "status": "complete_MV9_additive_moment_assembly_and_information_gate",
        "protocol_sha256": _sha256(protocol_path()),
        ...
        "q_provenance_computation_dtype": str(np.dtype(AUDIT_DTYPE)),
        "training_tensor_dtype": str(np.dtype(TRAINING_DTYPE)),
        "q_provenance_root_cause_repaired": (
            "no_float32_normalize_then_redimensionalize_round_trip_before_audit"
        ),
    ...
    print(f"MV9_INFORMATION_GATE={decision}", flush=True)
    return summary
...
def _train_model(
    ...
    *,
    ...
    channel_weights = torch.as_tensor(
        CHANNEL_LOSS_WEIGHTS, dtype=torch.float32, device=device
    ).view(1, -1, 1, 1)
    ny, nx = int(train_y.shape[-2]), int(train_y.shape[-1])
    ycoord = torch.linspace(0.0, 1.0, ny, device=device).view(ny, 1)
    xcoord = torch.linspace(0.0, 1.0, nx, device=device).view(1, nx)
    ...
    spatial_weights = (
        1.0
        + BOUNDARY_LOSS_STRENGTH
        * torch.exp(-distance / BOUNDARY_DECAY_FRACTION)
    ).view(1, 1, ny, nx)
    combined_weights = channel_weights * spatial_weights
    combined_weights = combined_weights / torch.mean(combined_weights)

    def loss_function(prediction, target):
        """Linear weighted MSE keeps the cross-seed Noise2Noise optimum unbiased."""
        squared = (prediction - target) ** 2
        pixel = torch.mean(combined_weights * squared)
        grad_x_squared = (
        ...
        grad_x = torch.mean(combined_weights[..., 1:] * grad_x_squared)
        grad_y = torch.mean(combined_weights[..., 1:, :] * grad_y_squared)
        return pixel + GRADIENT_LOSS_WEIGHT * (grad_x + grad_y)
    ...
    if best_state is None:
        raise RuntimeError("MV9 training produced no checkpoint")
    ...
    return model, {
        ...
        "loss_contract": {
            "type": "cross_seed_linear_weighted_MSE_plus_gradient_MSE",
            "channel_weights_tau_normal_qx_qy": CHANNEL_LOSS_WEIGHTS.tolist(),
            "boundary_loss_strength": BOUNDARY_LOSS_STRENGTH,
            "boundary_decay_fraction": BOUNDARY_DECAY_FRACTION,
            "gradient_loss_weight": GRADIENT_LOSS_WEIGHT,
            "adversarial_or_perceptual_loss": False,
        },
        "history": history,
    }
...
def run_model_task(
    ...
    *,
    ...
    if architecture not in ARCHITECTURES or training_seed not in TRAINING_SEEDS:
        raise ValueError("MV9 model task is outside the locked matrix")
    output_root = Path(output_root)
    ...
    if assembly["decision"] != "proceed_to_MV9_B1_kinetic_models":
        skipped = {
            "stage": STAGE,
            "status": "skipped_MV9_model_after_failed_information_gate",
            "architecture": architecture,
    ...
    summary = {
        "stage": STAGE,
        "status": "complete_MV9_B1_kinetic_model_task",
        "architecture": architecture,
        "training_seed": training_seed,
        "protocol_sha256": _sha256(protocol_path()),
        "training": training,
        ...
        "checks": checks,
        "decision": "accept_MV9_model_task" if all(checks.values()) else "hold_MV9_model_task",
    }
...
def _verify_task(directory: Path, architecture: str, seed: int) -> tuple[dict[str, Any], np.ndarray]:
    ...
    for name, record in manifest["files"].items():
        ...
        if (
            ...
            raise ValueError(f"MV9 task artifact failed verification: {path}")
    summary = json.loads((directory / "summary.json").read_text(encoding="utf-8"))
    if (
        summary.get("status") != "complete_MV9_B1_kinetic_model_task"
        or summary.get("decision") != "accept_MV9_model_task"
        or summary.get("architecture") != architecture
        or int(summary.get("training_seed", -1)) != seed
    ):
        raise ValueError(f"MV9 task summary contract failed: {directory}")
...
def _physical_figure(
    ...
    png = output / f"mv9_{key}_B1_vs_B10_physical_contours.png"
    pdf = output / f"mv9_{key}_B1_vs_B10_physical_contours.pdf"
...
def _create_archive(output_root: Path, names: Sequence[Path]) -> Path:
    ...
    archive = Path.home() / f"MOHAMMADZADEH_MV9_HEAT_FLUX_NOISE2NOISE_{tag}.zip"
    ...
    if archive.stat().st_size > 450 * 1024 * 1024:
        raise RuntimeError("MV9 return archive exceeds the 450 MiB upload limit")
    return archive
...
def run_post(output_root: Path) -> dict[str, Any]:
    ...
    if assembly["decision"] != "proceed_to_MV9_B1_kinetic_models":
        ...
        if skipped != len(ARCHITECTURES) * len(TRAINING_SEEDS):
            raise ValueError("MV9 audit-only path is missing guarded task records")
        summary = {
            "stage": STAGE,
            "status": "complete_MV9_information_audit_without_model_training",
            "assembly_decision": assembly["decision"],
            "skipped_model_tasks": skipped,
            "decision": "MV9_kinetic_moment_pilot_not_authorized_by_information_gate",
        }
        ...
        for architecture in ARCHITECTURES:
            for seed in TRAINING_SEEDS:
                ...
                if prediction.shape != raw.shape:
                    raise ValueError("MV9 task prediction shape differs from dataset")
        ...
        for method, records in per_seed.items():
            ...
            primary_records[method] = {
                ...
                "mean_heat_flux_composite_nrmse": float(
                    np.mean(
                        [item["heat_flux_composite_nrmse"] for item in values]
                    )
                ),
        ...
        for architecture in ARCHITECTURES:
            ...
            composite_ratio = current["mean_composite_nrmse"] / max(
                raw10_primary["mean_composite_nrmse"], 1e-12
            )
            heat_flux_ratio = current["mean_heat_flux_composite_nrmse"] / max(
                raw10_primary["mean_heat_flux_composite_nrmse"], 1e-12
            )
            improved_all = sum(
            ...
            q_field_ratios = {
                field: current["mean_per_field_nrmse"][field]
                / max(raw10_primary["mean_per_field_nrmse"][field], 1e-12)
                for field in OUTPUT_FIELDS[2:]
            }
            q_fields_within_cap = sum(
                value
                <= float(
                    protocol["analysis_contract"][
                        "maximum_individual_heat_flux_ratio_to_raw_B10"
                    ]
                )
                for value in q_field_ratios.values()
            )
            success[architecture] = {
                "composite_ratio_to_raw_B10": composite_ratio,
                "heat_flux_composite_ratio_to_raw_B10": heat_flux_ratio,
                "individual_heat_flux_ratios_to_raw_B10": q_field_ratios,
                "fields_no_worse_than_raw_B10": improved_all,
                "heat_flux_fields_within_locked_ratio_cap": q_fields_within_cap,
                "passes_pilot_rule": bool(
                    heat_flux_ratio
                    <= float(
                        protocol["analysis_contract"][
                    ...
                    and q_fields_within_cap == 2
                    and composite_ratio
                    <= float(
                        protocol["analysis_contract"][
                            "maximum_all_moment_composite_ratio_to_raw_B10"
                        ]
                    )
                ),
            }
        ...
        if np.count_nonzero(mask) != 1 or np.count_nonzero(mask10) != 1:
            raise ValueError("locked MV9 representative contour identity is absent")
        ...
        if not np.array_equal(reference, target10[index10]):
            raise ValueError("MV9 representative B1/B10 cross-fit targets differ")
        figure_methods = {
            "raw_b1": raw[index],
        ...
        if not np.allclose(scales[index], scales10[index10], rtol=1e-12, atol=0.0):
            raise ValueError("MV9 representative B1/B10 physical scales differ")
        figure_records = []
        ...
        metrics_path = output_root / "mv9_primary_condition_metrics.csv"
        ...
        summary = {
            "stage": STAGE,
            "status": "complete_MV9_kinetic_moment_feasibility_pilot",
            "protocol_sha256": _sha256(protocol_path()),
            ...
            "decision": (
                "MV9_feasibility_pass_proceed_to_separately_locked_full_confirmatory_stage"
                if any(value["passes_pilot_rule"] for value in success.values())
                else "MV9_feasibility_does_not_support_one_block_kinetic_moment_claim"
            ),
        }
        _atomic_json(output_root / "summary.json", summary)
        generated.append(output_root / "summary.json")
    ...
    manifest = {
        "stage": STAGE,
        "status": "complete_MV9_return_artifact_manifest",
        "files": {
    ...
    for name, record in manifest["files"].items():
        ...
        if _sha256(path) != record["sha256"] or path.stat().st_size != record["size_bytes"]:
            raise ValueError(f"MV9 recursive return verification failed: {path}")
    verification = {
        "stage": STAGE,
        "status": "complete_MV9_recursive_return_verification",
        "decision": "verified",
    ...
    print(f"MV9_OUTPUT_ROOT={output_root}")
