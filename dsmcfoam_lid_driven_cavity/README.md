...
The dictionaries follow the OpenFOAM Foundation v10 `dsmcFoam` tutorial syntax. The Unity job script first uses an already-loaded OpenFOAM environment, then tries common module names and installation paths. If your installation has a non-standard path, submit with `DSMCFOAM_BASHRC=/absolute/path/to/OpenFOAM/etc/bashrc`.
