...
BRANCH="${DSMC_BOOK_BRANCH:-agent/sparta-kn01-hq}"
...
if [[ -d "${REPO_DIR}/.git" ]]; then
  if [[ -n "$(git -C "${REPO_DIR}" status --porcelain --untracked-files=no)" ]]; then
    echo "The dedicated HQ clone has tracked changes; refusing to update it: ${REPO_DIR}" >&2
    exit 3
  fi
...
ARRAY_SUBMISSION="$(
  sbatch --parsable \
    ...
    --export="ALL,SPARTA_CASE_ROOT=${ROOT_DIR},UNITY_OPENMPI_MODULE=${OPENMPI_MODULE}" \
    hpc/unity_sparta_kn01_hq_array.slurm
)"
...
COLLECT_SUBMISSION="$(
  sbatch --parsable \
    ...
    hpc/unity_sparta_kn01_hq_collect.slurm
)"
COLLECT_JOB_ID="${COLLECT_SUBMISSION%%;*}"
STATE_FILE="${BASE_DIR}/LAST_SPARTA_KN01_HQ_JOBS.env"
BUNDLE="${ROOT_DIR}/runs/SPARTA_KN01_HQ_RESULTS_${ARRAY_JOB_ID}.tar.gz"

{
  printf 'HQ_BUILD_JOB_ID=%q\n' "${BUILD_JOB_ID}"
  printf 'HQ_ARRAY_JOB_ID=%q\n' "${ARRAY_JOB_ID}"
  printf 'HQ_COLLECT_JOB_ID=%q\n' "${COLLECT_JOB_ID}"
  printf 'HQ_CAMPAIGN_ROOT=%q\n' "${ROOT_DIR}"
  printf 'HQ_RETURN_BUNDLE=%q\n' "${BUNDLE}"
} > "${STATE_FILE}"

echo
echo "Submitted high-statistics SPARTA Kn=0.1 ensemble."
echo "Configuration: 200x200, 128 PPC, 40000 warmup + 160000 averaging steps, 3 seeds."
...
echo '  squeue -j "${HQ_BUILD_JOB_ID},${HQ_ARRAY_JOB_ID},${HQ_COLLECT_JOB_ID}"'
echo '  sacct -X -j "${HQ_BUILD_JOB_ID},${HQ_ARRAY_JOB_ID},${HQ_COLLECT_JOB_ID}" --format=JobID%22,JobName%20,State,ExitCode,Elapsed,NodeList%22'
echo
